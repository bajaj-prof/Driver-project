...
#define IO_PIN_COUNT_PER_PORT (8u)

/* We define pointer arrays of all the required port configuration registers.
 * Now to access a particular port register we need to extract the port number and then
 * use the corresponding array element [1] or [2] for P1 and P2
 */
...
static volatile uint8_t *const port_psel2_regs[IO_PORT_CNT] = { &P1SEL2, &P2SEL2};


#define UNUSED_CONFIG { IO_SELECT_GPIO,IO_DIR_OUTPUT,IO_RESISTOR_ENABLED,IO_OUT_LOW }

// This array hold the initial configuration of all IO pins
static const struct io_config io_initial_configs[IO_PORT_CNT * IO_PIN_COUNT_PER_PORT]= {
 //Output
  [IO_TEST_LED] = {IO_SELECT_GPIO, IO_DIR_OUTPUT, IO_RESISTOR_DISABLED, IO_OUT_LOW},

  // UART RX/TX
  [IO_UART_RXD] = {IO_SELECT_ALT3, IO_DIR_OUTPUT, IO_RESISTOR_DISABLED , IO_OUT_LOW},
  [IO_UART_TXD] = {IO_SELECT_ALT3, IO_DIR_OUTPUT, IO_RESISTOR_DISABLED , IO_OUT_LOW},
...
void io_init(void)
{
    ...
    {
        io_configuration (io , &io_initial_configs[io]);
    }
}
...
/* To extract the port and pin number from io_e we use the generic enums and then mask the last 3 bits
 * to get pin number and the next 2 bits (Bit 4 and 5) to get Port number
 */
