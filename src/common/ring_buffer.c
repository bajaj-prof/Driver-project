...
// Adds a new element to the inx pointed by head and then increments head. So that it points to the
// next empty loc
// Note: the function does not check if the buffer if the buffer is full. That code must be
// implemented in Main
...
// Reads an element from the tail position of the ring_buffer. Increments the tail so that it points
// to the next loc. Note: This function does not check if the buffer is empty. That code must be
// implemented in Main
